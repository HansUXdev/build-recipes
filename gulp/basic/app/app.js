...
gulp.task('app:clean:partials', function(cb) {
...
gulp.task('app:copy:templates', ['javascript'], function() {
  var config = [];
...
gulp.task('app:copy:partials', ['clean:partials'], function(cb) {
...
gulp.task('app:sass:settings', function() {
  octophant([
    'scss/_includes.scss',
    'scss/_global.scss',
...
gulp.task('app:server:start', function() {
  $.connect.server({
    root: './build',
    middleware: function() {
      return [
...
gulp.task('app:test:karma', ['build', 'sass'], function() {
  var testFiles = [
...
gulp.task('app:test:sass', function() {
...
gulp.task('app:test', ['test:karma', 'test:sass'], function() {
  console.log('Tests finished.');
});

// Motion testing

gulp.task('app:test:motion:compile', ['clean', 'sass', 'javascript'], function() {
...
gulp.task('app:test:motion', ['server:start', 'test:motion:compile'], function() {
...
gulp.task('app:deploy:dist', ['clean:dist'], function(cb) {
  var filter = $.filter(['*.css']);

  gulp.src('app:scss/foundation.scss')
    .pipe($.sass({
      outputStyle: 'nested',
      errLogToConsole: true
    }))
...
gulp.task('app:deploy:docs', ['build'], function() {
...
// Deploy to CDN
gulp.task('app:deploy:cdn', ['deploy:dist'], function() {
...
gulp.task('app:default', ['build', 'server:start'], function() {
